...
x <- list(name = "sc", discount = 0.1, states = c(pomdp_inuts_with_absorbing_states$states), 
          actions = c(actions), observations = c(pomdp_inuts_with_absorbing_states$states), 
          transition_prob = pomdp_inuts_with_absorbing_states$transition_probs, 
          observation_prob = pomdp_inuts_with_absorbing_states$observation_probs, 
          reward = pomdp_inuts_with_absorbing_states$rewards)
...
plot_policy_graph_kp<- function (x, belief = NULL, engine = c("igraph", "visNetwork"), 
          show_belief = TRUE, state_col = NULL, legend = TRUE, simplify_observations = TRUE, 
  ...
  switch(engine, igraph = .plot.igraph(x, belief, show_belief = show_belief, 
                                       state_col = state_col, legend = legend, simplify_observations = simplify_observations, 
                                       remove_unreachable_nodes = remove_unreachable_nodes, 
                                       ...), visNetwork = .plot.visNetwork(x, belief, show_belief = show_belief, 
                                                                           legend = legend, state_col = state_col, simplify_observations = simplify_observations, 
                                                                           remove_unreachable_nodes = remove_unreachable_nodes, 
                                                                           ...))
}

plot_igraph_kp<- function (x, belief = NULL, show_belief, state_col, legend, simplify_observations, 
                           remove_unreachable_nodes, edge.curved = NULL, ...) 
{
  pg <- policy_graph(x, belief, show_belief = show_belief, 
                     state_col = state_col, simplify_observations = simplify_observations, 
                     remove_unreachable_nodes = remove_unreachable_nodes)
  ...
  if (legend && show_belief && !is.null(vertex_attr(pg)$pie)) {
    legend("topright", legend = x$states, title = "Belief", 
           bty = "n", col = vertex_attr(pg)$pie.color[[1]], 
           pch = 15)
  }
}
